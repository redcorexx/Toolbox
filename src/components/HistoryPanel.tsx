...
import { ArrowLeft, History, RotateCcw, Star, Trash2 } from "lucide-react";
import {
  langByCode,
  timeAgo,
  toFa,
  ...
  return (
    <section id="history" className="scroll-mt-24 pt-12">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        {items.length > 0 && (
          <button
            onClick={() => {
              if (armClear) {
                ...
                setArmClear(true);
                notify("برای تایید پاک‌سازی، دوباره بزن", "info");
              }
            }}
            className={cn(
          ...
          >
            ...
            {armClear ? "مطمئنی؟ پاک کن!" : "پاک کردن همه"}
          </button>
        )}
      </div>
      ...
      <div className="mt-4 flex gap-2">
        {(
          [
            { key: "all", label: `همه (${toFa(items.length)})` },
            { key: "fav", label: `علاقه‌مندی‌ها (${toFa(favs.length)})` },
          ] as const
        ).map((t) => (
          <button
            key={t.key}
      ...
      {list.length === 0 ? (
        <div className="mt-4 flex flex-col items-center rounded-3xl border-2 border-dashed border-pine-900/12 px-6 py-14 text-center dark:border-white/12">
          ...
          <p className="mt-4 text-[15px] font-black text-pine-950 dark:text-white">
            {tab === "fav" ? "هنوز چیزی را نشان نکرده‌ای" : "هنوز ترجمه‌ای انجام نشده"}
          </p>
          <p className="mt-1 max-w-sm text-sm leading-7 text-ink/50 dark:text-white/50">
            {tab === "fav"
              ? "روی ستاره هر ترجمه بزن تا اینجا ذخیره شود و همیشه در دسترست باشد."
              : "اولین متنت را بالا بنویس تا ترجمه شود و تاریخچه‌ات اینجا ساخته شود."}
          </p>
        </div>
      ) : (
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-1.5 text-xs font-black">
                    ...
                    <ArrowLeft className="h-3.5 w-3.5 text-ink/35 dark:text-white/35" />
                ...
                <div className="mt-3 flex items-center gap-1.5 border-t border-dashed border-pine-900/10 pt-3 dark:border-white/10">
                  ...
                  <span className="mr-auto flex items-center gap-1">
                    <button
                      ...
                      title={h.fav ? "حذف از علاقه‌مندی‌ها" : "افزودن به علاقه‌مندی‌ها"}
